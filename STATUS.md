# 작업 핸드오프 — 백테스트 진행 상황

> 회사·집 PC 전환 시 컨텍스트 복구용. Claude Code 새 세션이 이 파일을 먼저
> 읽고 시작하면 진행 상태가 그대로 복원된다. 진행이 한 단계 끝날 때마다
> 이 파일을 갱신한다.

**마지막 갱신:** 2026-05-28
**현재 단계:** ✓ 전략 v1.0 후보 확정 (SRT C1) — 봇 개발 단계로 이동

---

## TL;DR

**SRT C1 후보 OOS 통과. 전략 v1.0 후보 확정.**

후보 파라미터: `SRTParams(trix_required=False, rr=1.5, swing_lookback=20)`
타임프레임: 1h main, HTF 없음 (자막 정의 그대로)

| 지표 | IS (12,264봉) | OOS (5,256봉) | 비교 |
|---|---|---|---|
| 수익률 | +109.23% | **+53.67%** | 양수 유지 |
| MDD | -26.82% | **-16.80%** | OOS 가 더 낮음 |
| PF | 1.44 | **1.60** | OOS 가 더 높음 |
| 승률 | 48.7% | 47.8% | 거의 동일 |
| 거래수 | 236 | 92 | 기간 비례 |

OOS PF (1.60) > IS PF (1.44) — 과최적화 아님, 메커니즘에 진짜 edge.
Walk-forward K=4 도 통과 (양수율 75%, 평균 PF 1.52).

**이 데이터셋의 OOS 호출은 끝.** v0.1 폐기(2026-05-28 오전) + SRT C1
검증(같은 날 오후) 까지 OOS 2회 사용. 같은 OOS 구간을 더 만지지 않음.

**다음 단계:** 설계서 §5 의 9개 모듈 봇 개발 → 테스트넷 가동 → 실거래.
또는 paper trade 로 미래 데이터 검증.

---

## 바로 다음 명령

전략 검증은 끝. 다음 세션은 **봇 개발 단계** 진입:

```
설계서 §5 의 9개 모듈 봇 개발 들어가자. SRT C1 후보 확정됐고
파라미터는 strategy/yt_strategies.py 에 박혀 있어.
구조부터 잡아보자.
```

또는 paper trade 먼저:

```
SRT C1 으로 paper trade 환경 만들자. 바이낸스 테스트넷 + 실시간 1h 캔들
폴링, 신호 발생 시 마켓 주문 시뮬.
```

---

## 시도 정리 (전체 흐름)

| # | 시도 | IS 결과 | 결론 |
|---|------|--------|------|
| 1 | v0.1 baseline (15m, crossover, partial_tp) | -33.7% / PF 0.85 | 메커니즘 자체 적자 |
| 2 | 15m 그리드 서치 48조합 | 모두 음수, best -16.6% | 15m 에선 어떤 파라미터도 양수 불가 |
| 3 | extension entry (CCI 임계 위 지속+가속) | -43.2% | 기각. "꼭대기에서 진입" 가설 틀림 |
| 4 | trailing only (15m, ATR×2) | -28.9% / PF 0.90 | 15m 비용 못 이김 |
| 5 | 1h main + 4h HTF baseline | -6.2% / PF 0.99 | 돌파. TF 한 칸이 결정타 |
| 6 | 1h + CCI ±130 + trailing ATR×2 (v0.1 후보) | +6.74% / PF 1.29 (IS) | OOS -9.99% / PF 0.48 → 폐기 |
| 7 | YT 4전략 비교 (WM/DBL_BOTTOM/DBL_TOP/SRT) | 모두 IS 음수, best SRT -6.57% / PF 1.05 | SRT 만 PF>1, 튜닝 진행 |
| 8 | SRT 그리드 18조합 (trix/rr/swing) | best +109% / PF 1.44 (IS) | 13/18 양수, TRIX 필터 제거가 핵심 |
| 9 | SRT Walk-forward K=4 (C1/C2/C3) | C1 양수율 75%, 평균 PF 1.52 | C1 일관성 최우수 |
| 10 | **SRT C1 OOS 검증** | **+53.67% / PF 1.60 / MDD -16.8%** | **v1.0 후보 확정** |

상세 수치·코드는 다음 커밋:
- `3a924e7` — strategy v0.1 + engine
- `ac249f0` — 15m 48조합 그리드 서치
- `2b363e9` — extension + trailing 시도 (기각)
- `b688dcd` — 타임프레임 일반화 + 1h 결과
- `ce961f5` — OOS 검증 스크립트 (v0.1 locked-in 후보)
- `ba20064` — STATUS.md 핸드오프 문서
- `232b33b` — v0.1 OOS 실패 기록 + 4전략 방향 전환
- `adac71c` — YT 4전략 (WM/DBL_BOTTOM/DBL_TOP/SRT) + 엔진 + IS 비교
- `5071f76` — SRT 그리드 (18조합), TRIX 옵션화, top +109%
- `671c701` — SRT walk-forward (K=4), C1 일관성 검증
- (이번 커밋) — SRT C1 OOS 통과, v1.0 후보 확정

---

## 중요 규칙·제약

### OOS는 단 한 번 (명세 §6.2) — 이미 종료
같은 OOS 구간(2025-09-24 ~ 2026-04-30)이 두 번 사용됨:
  - 2026-05-28: v0.1 후보 (verify_oos.py) → -9.99% / PF 0.48 → 폐기
  - 2026-05-28: SRT C1 (verify_oos_srt_c1.py) → +53.67% / PF 1.60 → v1.0 확정

**더 이상 같은 OOS 구간으로 backtest 안 함.** 이후 검증은 paper trade 나
실거래(소액)로만. 새 데이터(2026-05 이후)가 누적되면 그것이 또 다른 OOS 가 됨.

### 운영·코드 규칙
- 파이썬 표준 출력 UTF-8 강제 — 모든 백테스트·다운로드 스크립트 상단에서
  `sys.stdout.reconfigure(encoding="utf-8", errors="replace")` 처리됨. 그래도
  PowerShell 에서 깨지면 `$env:PYTHONUTF8=1`. `errors="replace"` 는 orphan
  surrogate (Claude Code API 가 거부) 차단용.
- 손절은 거래소 측 STOP_MARKET 시뮬레이션. 인트라바 H/L 가 손절가를
  관통하면 손절. SL 과 TP 가 같은 캔들이면 SL 우선 (명세 §4 보수).
- 신호는 캔들 close 에서 생성, 진입은 다음 캔들 open. look-ahead 차단.
- 트레일링 모드에서 손절은 **한 방향으로만** 이동 (롱이면 위로만). 설계서 §3
  "손절 이동·해제 금지" 원칙의 안전한 해석.

---

## 회사·집 환경 차이로 인한 주의

- **데이터 파일** (`data/BTCUSDT-*.csv`) 은 `.gitignore` 처리됨. 각 PC 에서
  `python scripts/download_binance_data.py` 로 직접 받아야 함. 같은 코드·기간
  이라 결과는 비트 단위 동일.
- **백테스트 결과** (`data/backtest_results/*.csv`) 도 gitignored. 재실행으로
  재생성. IS 결과는 결정론적이라 동일.
- **`.env`** 는 `.env.example` 복사. 백테스트만 할 거면 키 빈칸 OK.
  `RUN_MODE=testnet` 확인.
- **`.venv`** 는 PC 마다 새로. `python -m venv .venv` + `pip install -r requirements.txt`.
- **Python 자체 설치 — 회사 PC LTSC 정책 주의:** Windows 10 Enterprise LTSC
  에서는 python.org MSI installer 가 그룹정책에 막혀 0x80070003 으로 실패함
  (Package Cache 에 core.msi 가 안 캐싱됨). 우회: **`uv`** standalone exe 로
  설치.
  ```powershell
  # 1) uv 받기 (단일 exe, 설치 불필요)
  curl -sSL -o $env:TEMP\uv.zip https://github.com/astral-sh/uv/releases/latest/download/uv-x86_64-pc-windows-msvc.zip
  Expand-Archive $env:TEMP\uv.zip C:\Users\$env:USERNAME\tools\uv
  # 2) standalone Python 3.12 추출 (MSI 안 씀)
  C:\Users\$env:USERNAME\tools\uv\uv.exe python install 3.12
  # 3) venv 생성 (위에서 받은 python 사용)
  & "$env:APPDATA\uv\python\cpython-3.12.13-windows-x86_64-none\python.exe" -m venv .venv
  ```
- **git config** (user.name / user.email) 은 글로벌로 이미 설정돼 있음
  (seokh / kyuhyun.seo@kweather.co.kr). 새 PC 면 다시 설정 필요.
- **PowerShell ExecutionPolicy** — `Set-ExecutionPolicy -Scope CurrentUser -ExecutionPolicy RemoteSigned`
  를 PC 마다 한 번. (안 그러면 `Activate.ps1` 실행 안 됨)

---

## 새 Claude Code 세션 시작 시 권장 첫 문장

```
seocoinbot 백테스트 작업 이어가는 거야. STATUS.md 와 git log -8 읽어보고
현재 상태 파악한 다음, 다음 단계 진행해줘.
```

이걸로 컨텍스트 복원됨. STATUS.md 의 "바로 다음 명령" 섹션에 정확한 명령
박혀 있음.

---

## 아직 안 한 일

- 9개 모듈 봇 개발 (설계서 §5) — **다음 단계**
  - 시장 데이터, 전략, 리스크, 실행, 동기화, 검증, 로깅, 모니터링, 킬스위치
  - SRT C1 신호 함수는 strategy/yt_strategies.py 에 이미 있음 — 그대로 봇에 끼움
- 바이낸스 테스트넷 가동 — 봇 완성 후
- 실거래 (아주 작은 금액) — 테스트넷 검증 후

설계서 §8 의 흐름 그대로.
